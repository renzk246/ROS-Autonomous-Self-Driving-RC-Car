...
def main():
    ...
    while not rospy.is_shutdown():
        if received_img:
            ...
            # IMAGE MANIPULATION
            naturalImage = np.asarray(cv_img.copy())
            ...
            naturalImage = np.array([naturalImage])
            laneImage = np.array([laneImage])
            x = tf.constant(naturalImage)
            y = tf.constant(laneImage)

            # IMAGE MANIPULATION TIME
            startTime = time.time()

            # INFERENCE
            result = infer(input_1=x, input_2=y)
            ...
            imgDisplay = cv2.putText(imgDisplay, "Steering", (0, 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1,
                                     cv2.LINE_AA)

            color = (255, 0, 255)
            imgDisplay = throttleMeter(imgDisplay, color, throttle, meterPos=5, txtPos=5)
            imgDisplay = cv2.putText(imgDisplay, "Throttle", (0, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1,
                                     cv2.LINE_AA)
        ...
        msg2 = bridge.cv2_to_imgmsg(imgDisplay, 'bgr8')

        msg3 = Extra()
        msg3.inference_drive = inferenceTime
...
if __name__ == "__main__":
    ...
    pub2 = rospy.Publisher("/img_drive", Image, queue_size=2)
    pub3 = rospy.Publisher("/monitor", Extra, queue_size=1)
